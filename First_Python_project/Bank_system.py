...
from tkinter import messagebox, Button, Tk, Label, Entry, Frame, Toplevel, StringVar, IntVar, LEFT, CENTER, mainloop,DISABLED
import webbrowser
...
if conn:
    ...
    def post_login():
        ...
        def toggle_login():
            ...
            def update_info():
                ...
                psswd = Entry(toggle, width=23, border=0, textvariable=temp_passwd, bg='#94BCF9', show="*",
                              font=("calibri", 15))
                ...
                psswd = Entry(toggle, width=23, border=0, textvariable=tem_newpasswd, bg='#94BCF9',
                              font=("calibri", 15), show="*")
                psswd.insert(0, "")
                psswd.place(x=950, y=550)
                Frame(toggle, width=230, height=2, bg="black").place(x=950, y=578)
                Button(toggle, image=apply, command=check, border=0, bg='#94BCF9', activebackground='#94BCF9').place(
                    x=690, y=630)
            ...
            def Withdraw():
                ...
                Label(toggle, text="Enter The amount", bg="#C4C4C4", fg="#262626", font=("Roboto", 20)).place(x=850,
                                                                                                              y=450)
                Entry(toggle, width=23, border=0, textvariable=temp_withdraw, bg="#C4C4C4", font=("calibri", 15)).place(
                    x=850, y=500)
                Frame(toggle, width=230, height=2, bg="black").place(x=850, y=528)

                Button(toggle, image=withdraw_button, bg="#C4C4C4", border=0, activebackground="#C4C4C4",
                       command=accountchecking).place(x=900, y=550)
            ...
            def transfer_money():
                ...
                Frame(toggle, width=230, height=2, bg="black").place(x=850, y=528)
                # account entry
                Label(toggle, text="Enter account Username", bg="#C4C4C4", fg="#262626", font=("Roboto", 20)).place(
                    x=825, y=560)
                Entry(toggle, width=23, border=0, textvariable=temp_transed, bg="#C4C4C4", font=("calibri", 15)).place(
                    x=850, y=610)
    ...
    def regButt():
        ...
        if int(reg_balance.get()) == reg_balance.get():
            regbalance = int(reg_balance.get())
            if reguser == "" or regpassowrd == "" or regemail == "" or regbalance == "":
                ...
                if (re.search(pattern, regemail)):
                    if (re.search(pattern2, reguser)):
                        ...
                        messagebox.showwarning("Wrong Info",
                                               "Username length should be at least 8 letters. \n **Username can contain  '_ / . ' but no at the beginning \n or the end of it")
                else:
    ...
    def Login():
        ...
        Label(window, width=41, text="P A S S W O R D", bg='#94BCF9', fg="#262626", font=('Roboto', 18)).place(x=705,
                                                                                                               y=600)
        Entry(window, width=23, font=('Roboto'), textvariable=login_passwd, border=0, bg='#94BCF9', show='*').place(
            x=830,
            y=660)
    ...
    def Reg():
        ...
        Label(window, width=41, text="P A S S W O R D", bg='#94BCF9', fg="#262626", font=('Roboto', 18)).place(x=705,
                                                                                                               y=500)
        Entry(window, width=23, font=('Roboto'), textvariable=reg_password, border=0, bg='#94BCF9', show='*').place(
            x=830,
            y=560)
        ...
        Label(window, width=41, text="B A L A N C E", bg='#94BCF9', fg="#262626", font=('Roboto', 18)).place(x=705,
                                                                                                             y=760)
